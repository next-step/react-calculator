...
export const useCalculator = () => {
    ...
    const operate = (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        if(isOperator(currentOperation)){
            if(isFirstNumberNotExist){   // 연산후 결과를 다시 계산하기위해 필요한 조건문
            ...
            const operator=operation as OperatorType
            const calcualtionResult=caculateNumber(operator,firstNumber,currentNumber)
            setResult(calcualtionResult)
            setFirstNumber(parseInt(calcualtionResult))
            setCurrentNumber(0)
        }

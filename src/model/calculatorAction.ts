...
export const getCalculatorAction = (state: string, input: CalculatorArgs) => {
  ...
  if (Number(input)) {
    if (isDigitSizeOver(state, input)) {
...
const isDigitSizeOver = (state: string, input: CalculatorArgs) => {
  const lastNumber = getCalculatorNumberArray(state + String(input)).at(-1);
  return lastNumber!.length > MAX_NUMBER_SIZE;
};
